...
- Always generate immediately without asking follow-up questions.`
      ...
      {
        role: "user",
        content: `Generate variation ${variationIndex + 1} for pages ${pageRange[0]}-${pageRange[1]} using this context:\n\n${contextText}\n\nAlready generated variations (do not repeat wording or composition):\n${existingPrompts.join("\n---\n") || "None yet."}`
      }
    ],
...
export async function generatePrompts(
  ...
  for (let i = 0; i < 3; i++) {
    try {
      const text = await generateSinglePrompt(contextText, pageRange, tone, forbiddenPhrases, i, promptTexts);
